	...
	<section>
		...
		<p>
			...
			if (!empty($query)) {
				...
				echo "<b>Activity Type:</b> $activity<br/>";
			} else {
				echo "<b>Target Id:</b> $target<br/>";
				$sql_activity="CREATE TEMP TABLE activity_data AS (
						SELECT md.molregno, ac.standard_value, fr.$fingerprint_cols
						FROM molecule_dictionary md, compound_records cr, activities ac, assays a, target_dictionary td, fps_rdkit fr
						WHERE md.molregno = cr.molregno AND cr.record_id = ac.record_id AND ac.assay_id = a.assay_id AND a.tid = td.tid AND md.molregno=fr.molregno
						AND td.chembl_id='$target' AND ac.standard_type='Ki' AND ac.standard_units='nM' 
						AND ac.standard_relation = '=' AND ac.standard_value IS NOT NULL)";
			}
			echo "<b>Similarity Threshold:</b> $threshold<br/>";
			...
			// execute query
 			...
 			echo "<h3>Activity cliffs for $compound</h3>";
 			$check=pg_fetch_row($result);
 			if (empty($check[0])){	 			
 					echo '<p><b>There are not cliff pairs for this query</b></p>';
 			}
 			else { 			
	 			$campos=array("Molecule 1","Molecule 2","Disparity","Similarity diff","Activity Mol1","Activity Mol2","Activity diff");
	 			echo "<table><tr>";
				...
				while ($row = pg_fetch_array($result)) {
	 				if (($row[cid1] == $compound) || ($row[cid2] == $compound)){
	 					echo "<tr>";
						//round($row[sim],2)
